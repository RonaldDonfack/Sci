...
st.set_page_config(page_title="Cycle d'Investissement · Calculateur de Capitaux", page_icon="🏗️", layout="wide")
...
def build_pyvis(nodes, edges, equity_vec, direct_assets, sci_shares,
                show_associés=True, show_effective=False,
  ...
  "interaction":{"hover":true,"tooltipDelay":100,"zoomView":true,"dragNodes":true}
    ...
    for node in nodes:
        ...
        title = (f"<div style='font-family:Inter;padding:8px;min-width:200px'>"
                 ...
                 f"<span style='color:#8B949E'>Actifs directs</span>: <b>{da:,.0f} €</b><br/>"
                 f"<span style='color:#8B949E'>Capitaux totaux</span>: <b style='color:#3FB950'>{eq:,.0f} €</b>"
    ...
    for src, dst, pct in edges:
        net.add_edge(src, dst, label=f"{pct*100:.2f}%", title=f"{src} détient {pct*100:.2f}% de {dst}",
                     width=1+pct*4, color={"color": "#30363D"})
    if show_associés and all_persons:
        ...
        for p_i, person in enumerate(all_persons):
            ...
            title = (f"<div style='font-family:Inter;padding:8px;min-width:210px'>"
                     ...
                     f"<span style='color:#8B949E'>Richesse totale</span>: <b style='color:#3FB950'>{wealth:,.0f} €</b>"
...
<div style='display:flex;align-items:center;gap:1rem;margin-bottom:1.5rem'>
  ...
  <div>
    <h1 style='margin:0;font-size:1.6rem;color:#E6EDF3'>Cycle d'Investissement · Calculateur de Capitaux</h1>
    <p style='margin:0;color:#8B949E;font-size:.85rem'>Propriété SCI · richesse des associés · sans double comptage</p>
  </div>
</div>
""", unsafe_allow_html=True)

tab_input, tab_results, tab_associés, tab_cashflow, tab_profile, tab_graph = st.tabs([
    "📋 Saisie des données", "📊 Capitaux SCI", "👥 Richesse Associés", "💶 Flux de Trésorerie", "🔍 Profil Associé", "🕸️ Graphe des Relations"
])

with tab_input:
    col1, col2, col3 = st.columns([2, 1.5, 2], gap="large")
    with col1:
        st.markdown('<div class="section-header">Relations de Propriété</div>', unsafe_allow_html=True)
        relations_text = st.text_area("Relations", value=DEFAULT_RELATIONS, height=300, label_visibility="collapsed")
        st.markdown('<div class="section-header">➕ Ajouter une SCI (simulation)</div>', unsafe_allow_html=True)
        nc1, nc2, nc3 = st.columns([2, 2, 1])
        with nc1: new_sci_name = st.text_input("Nom de la SCI", placeholder="LEM NOUVEAU", label_visibility="collapsed")
        with nc2: new_sci_assets = st.number_input("Actifs directs €", min_value=0, value=100000, step=10000, label_visibility="collapsed")
        with nc3:
            if st.button("＋ SCI"):
                if new_sci_name:
                    ...
                    st.success(f"Ajoutered {nm}")
        st.markdown('<div class="section-header">➕ Ajouter un Lien de Propriété</div>', unsafe_allow_html=True)
    ...
    with col2:
        st.markdown('<div class="section-header">Actifs Directs (€)</div>', unsafe_allow_html=True)
        assets_text = st.text_area("Assets", value=DEFAULT_ASSETS + st.session_state.get("extra_assets", ""),
                                   height=200, label_visibility="collapsed")
        st.markdown('<div class="section-header">Config Flux de Trésorerie</div>', unsafe_allow_html=True)
        st.caption("`SCI : revenus : %associés : %charges` — défaut 60/40")
        ...
        st.markdown('<div class="section-header">➕ Ajouter Flux SCI</div>', unsafe_allow_html=True)
        ...
        with acf4:
            if st.button("＋ CF"):
                if new_cf_sci:
                    ...
                    st.success("Ligne de flux ajoutée.")
    with col3:
        st.markdown('<div class="section-header">Parts des Associés</div>', unsafe_allow_html=True)
        associés_text = st.text_area("Partners", value=DEFAULT_PARTNERS, height=480, label_visibility="collapsed")
    ...
    if st.button("⚙️  Calculer", type="primary"):
        st.session_state.update({"calc_done": True, "relations_text": full_relations,
                                  "assets_text": assets_text, "associés_text": associés_text,
                                  "cf_text": cf_text})
        st.success("✅ Terminé — consultez les onglets de résultats.")

if "calc_done" not in st.session_state:
    st.session_state.update({"calc_done": True, "relations_text": DEFAULT_RELATIONS,
                              "assets_text": DEFAULT_ASSETS, "associés_text": DEFAULT_PARTNERS,
...
partners_raw = parse_associés(st.session_state.get("associés_text", DEFAULT_PARTNERS))
...
for _sci in nodes:
    ...
    sci_cf[_sci] = {"revenue": rev, "to_associés": rev * pp, "to_charges": rev * cp,
                    "partner_pct": pp, "charges_pct": cp}

pool_vec = np.array([sci_cf[_sci]["to_associés"] for _sci in nodes])
...
with tab_results:
    ...
    with k1: st.markdown(f"""<div class='metric-card'><div class='label'>Total Actifs du Groupe</div>
        <div class='value'>{total_group_assets:,.0f} €</div>
        <div class='sub'>Somme des actifs directs · sans double comptage</div></div>""", unsafe_allow_html=True)
    with k2:
        top_node = nodes[int(np.argmax(equity_vec))]
        st.markdown(f"""<div class='metric-card'><div class='label'>Plus Grande SCI (capitaux)</div>
    ...
    with k3: st.markdown(f"""<div class='metric-card'><div class='label'>SCIs Modélisées</div>
        <div class='value'>{len(nodes)}</div><div class='sub'>{len(edges)} liens de propriété</div></div>""", unsafe_allow_html=True)
    with k4: st.markdown(f"""<div class='metric-card'><div class='label'>Partners</div>
        <div class='value'>{len(all_persons)}</div><div class='sub'>dans toutes les SCIs</div></div>""", unsafe_allow_html=True)
    st.markdown("""<div class='info-box'>
    ⓘ <b>Capitaux Totaux</b> par SCI = actifs directs propres + part proportionnelle des filiales,
    résolus via (I−W)·capitaux = actifs_directs. Total groupe = somme des actifs directs uniquement.
    </div>""", unsafe_allow_html=True)
    st.markdown('<div class="section-header">Décomposition des Capitaux SCI</div>', unsafe_allow_html=True)
    rows = []
    for node in nodes:
        ...
        shares = sci_shares.get(node, {})
        rows.append({"SCI": node, "Actifs Directs (€)": da, "Total Equity (€)": eq,
                     "Coût Unitaire (€)": SCI_UNIT_COST.get(node, 0),
                     "Total Investi (€)": sci_total_inv.get(node, 0),
                     "Associés Personnes Physiques": sum(1 for m in shares if m not in set(nodes)),
                     "Détenu par SCIs": ", ".join(f"{o} ({p:.2f}%)" for o, p in owners) if owners else "—",
                     "Détient SCIs":     ", ".join(f"{o} ({p:.2f}%)" for o, p in owned) if owned else "—"})
    df = pd.DataFrame(rows).sort_values("Total Equity (€)", ascending=False).reset_index(drop=True)
    st.dataframe(
        df.style.format({"Actifs Directs (€)": "{:,.0f}", "Total Equity (€)": "{:,.0f}",
                          "Coût Unitaire (€)": "{:,.0f}", "Total Investi (€)": "{:,.0f}"})
               .background_gradient(subset=["Total Equity (€)"], cmap="Blues")
               .background_gradient(subset=["Actifs Directs (€)"], cmap="Greens")
               .background_gradient(subset=["Total Investi (€)"], cmap="Oranges"),
        use_container_width=True, height=340)
    with st.expander("🔍 Matrice de Détention Effective SCI→SCI"):
...
with tab_associés:
    ...
    with kp1: st.markdown(f"""<div class='metric-card'><div class='label'>Richesse Totale des Associés</div>
        <div class='value'>{total_person_wealth:,.0f} €</div>
        <div class='sub'>Somme pour {len(all_persons)} associés</div></div>""", unsafe_allow_html=True)
    with kp2: st.markdown(f"""<div class='metric-card'><div class='label'>Actifs Réels du Groupe</div>
        <div class='value'>{total_group_assets:,.0f} €</div>
        <div class='sub'>Total consolidé réel</div></div>""", unsafe_allow_html=True)
    with kp3: st.markdown(f"""<div class='metric-card'><div class='label'>Écart de Réconciliation</div>
        <div class='value' style='color:{"#3FB950" if rounding_gap<100 else "#F85149"}'>{rounding_gap:,.2f} €</div>
        <div class='sub'>{"✓ Arrondi uniquement" if rounding_gap<100 else "⚠ Vérifier les parts des associés"}</div></div>""",
        unsafe_allow_html=True)
    fc, _ = st.columns([2, 4])
    with fc:
        partner_filter = st.selectbox("Filter partner",
            options=["— All associés —"] + sorted(all_persons, key=lambda p: -person_wealth[P_idx[p]]),
            label_visibility="collapsed")
    sorted_persons = sorted(all_persons, key=lambda p: -person_wealth[P_idx[p]])
    display_persons = sorted_persons if partner_filter == "— All associés —" else [partner_filter]
    view_mode = st.radio("View", ["💰 Richesse %", "💰 Richesse €", "🥧 Camembert"],
                         horizontal=True, label_visibility="collapsed")

    if view_mode == "💰 Richesse %":
        st.markdown('<div class="section-header">Richesse des Associés — % des Actifs du Groupe</div>', unsafe_allow_html=True)
        cols = st.columns(3)
        for ri, person in enumerate(sorted_persons):
            ...
            with cols[ri % 3]:
                ...
                <div class='person-card'>
                  ...
                  <div style='font-size:.8rem;color:#8B949E;margin:.1rem 0'>{pw:,.0f} € sur {total_group_assets:,.0f} €</div>
                  <div class='bar-bg'><div class='bar-fg' style='width:{bar_pct}%'></div></div>
                  <div style='margin-top:.4rem'>{eff_badges(person)}</div>
                </div>""", unsafe_allow_html=True)

    elif view_mode == "💰 Richesse €":
        # ── CHANGE 1: show effective wealth (eff % × equity) per SCI, no % badges ──
        st.markdown('<div class="section-header">Richesse des Associés — € par détention effective par SCI</div>', unsafe_allow_html=True)
        cols = st.columns(3)
        for ri, person in enumerate(sorted_persons):
            if person not in display_persons: continue
            pi_p = P_idx[person]
            ...
            # Badges show Nom de la SCI + € amount from effective ownership
            sci_badges = "".join(
            ...
            with cols[ri % 3]:
                ...
                <div class='person-card'>
                  ...
                  <div style='font-size:.8rem;color:#8B949E;margin:.1rem 0'>{pw/total_group_assets*100:.2f}% du groupe</div>
                  <div class='bar-bg'><div class='bar-fg' style='width:{bar_pct}%'></div></div>
                  <div style='margin-top:.4rem'>{sci_badges}</div>
                </div>""", unsafe_allow_html=True)

    else:
        st.markdown('<div class="section-header">Distribution de Richesse — Camembert</div>', unsafe_allow_html=True)
        try:
            ...
            if len(display_persons) == 1:
                st.markdown(f"**Parts effectives :** {eff_badges(display_persons[0])}", unsafe_allow_html=True)
        except ImportError: st.info("Installez plotly.")

# ── Cash Flow Tab ──────────────────────────────────────────────────────────────
with tab_cashflow:
    st.markdown("""<div class='info-box'>
    ⓘ Le flux de trésorerie utilise <b>% de détention effective × pool augmenté</b>. La part de chaque associé
    par SCI = son % effectif total (direct + indirect) × le pool augmenté de cette SCI.
    Total distribué = total du pool fixe (conservé).
    </div>""", unsafe_allow_html=True)
    with st.expander("⚙️ Configurer le flux mensuel par SCI", expanded=False):
        cf_col1, cf_col2 = st.columns(2)
        with cf_col1:
            cf_edit = st.text_area("CF config", value=st.session_state.get("cf_text", DEFAULT_CASHFLOW),
                                   height=300, label_visibility="collapsed")
        with cf_col2:
            st.caption("""**Format :** `SCI : revenus : %associés : %charges`
```
LEM IMMO : 3000 : 60 : 40
```
- **revenus** = € mensuels générés par la SCI
- **partner%** = % distributed to associés (default 60)
- **%charges** = % pour les charges d'exploitation (défaut 40)""")
        if st.button("💾 Appliquer la config flux"):
            st.session_state["cf_text"] = cf_edit
            st.success("Mis à jour — cliquez sur Calculer pour actualiser.")
    total_rev   = sum(v["revenue"]     for v in sci_cf.values())
    total_to_p  = sum(v["to_associés"] for v in sci_cf.values())
    total_charg = sum(v["to_charges"]  for v in sci_cf.values())
    k1, k2, k3 = st.columns(3)
    for col, label, val, color, sub in [
        (k1, "Revenus Mensuels Totaux", total_rev,   "#58A6FF", f"{total_rev*12:,.0f} €/an"),
        (k2, "→ Associés (pool)",     total_to_p,  "#3FB950", f"{total_to_p*12:,.0f} €/an"),
        (k3, "→ Charges",             total_charg, "#F85149", f"{total_charg*12:,.0f} €/an"),
    ]:
    ...
    st.markdown('<div class="section-header">Revenus et Répartition par SCI</div>', unsafe_allow_html=True)
    try:
        import plotly.graph_objects as go
        fig_cf = go.Figure()
        fig_cf.add_trace(go.Bar(name="→ Partners", x=list(nodes),
                                y=[sci_cf[s]["to_associés"] for s in nodes], marker_color="#3FB950"))
        ...
        fig_cf.update_layout(barmode="stack", plot_bgcolor="#0D1117", paper_bgcolor="#0D1117",
                             ...
                             yaxis=dict(gridcolor="#21262D", title="€/moisis"),
                             xaxis=dict(gridcolor="#21262D"), margin=dict(t=50, b=40, l=60, r=20))
        st.plotly_chart(fig_cf, use_container_width=True)
    except ImportError: pass
    st.markdown('<div class="section-header">Distribution des Flux aux Associés</div>', unsafe_allow_html=True)
    cf_view = st.radio("Vue Flux", ["Cartes résumé", "Par SCI"], horizontal=True)
    sorted_persons_cf = sorted(all_persons, key=lambda p: -person_total_cf[P_idx[p]])
    if cf_view == "Cartes résumé":
            ...
            with cols3[rank % 3]:
                st.markdown(f"""<div class='person-card'>
                  ...
                  <div style='font-size:.77rem;color:#8B949E;margin:.3rem 0'>
                    ...
                    Annuel : <b style='color:#58A6FF'>{total_m*12:,.0f}€</b>
                  </div><div>{sci_bd}</div></div>""", unsafe_allow_html=True)
    else:
        sci_cf_select = st.selectbox("Sélectionner SCI", nodes, key="cf_sci_select", label_visibility="collapsed")
        pool_this = sci_cf[sci_cf_select]["to_associés"]
        ...
        with ca: ca.markdown(f"""<div class='metric-card'><div class='label'>Revenus Mensuels</div>
            <div class='value'>{sci_cf[sci_cf_select]['revenue']:,.0f} €</div>
            <div class='sub'>{pct_label} → associés</div></div>""", unsafe_allow_html=True)
        with cb: cb.markdown(f"""<div class='metric-card'><div class='label'>Pool Fixe Associés</div>
            <div class='value' style='color:#3FB950'>{pool_this:,.0f} €/mois</div></div>""", unsafe_allow_html=True)
        with cc: cc.markdown(f"""<div class='metric-card'><div class='label'>Pool Effectif (augmenté)</div>
            <div class='value' style='color:#58A6FF'>{pool_eff_this:,.0f} €/mois</div>
            <div class='sub'>dont réceptions inter-SCI</div></div>""", unsafe_allow_html=True)
        ...
        for member, pct in sorted(shares_this.items(), key=lambda x: -x[1]):
            ...
            rows_bysci.append({"Member": member, "Type": mtype, "Direct %": pct * 100,
                                ...
                                "CF direct (€/mois)": cash_d, "CF effective (€/mois)": cash_eff,
        ...
        try:
            ...
            vals_eff = [next(r["CF effective (€/mois)"] for r in rows_bysci if r["Member"] == m) for m in all_m]
            ...
            fig_bs.update_layout(plot_bgcolor="#0D1117", paper_bgcolor="#0D1117", font_color="#C9D1D9",
                yaxis=dict(gridcolor="#21262D", title="€/mois"), xaxis=dict(gridcolor="#21262D", tickangle=-30),
                title=dict(text=f"Flux effectif de {sci_cf_select} (% eff. × pool eff.)", font_size=13),
...
with tab_profile:
    ...
    for col, label, val, color, sub in [
        (k1, "Richesse Totale (directe)",   pw,            "#58A6FF", f"{pw/total_group_assets*100:.2f}% du groupe"),
        (k2, "Investissement Initial",      total_invested, "#8B949E", f"{len(person_inv_by_sci.get(selected_person, {}))} SCI(s)"),
        (k3, "Gain / Perte",             gain,          "#3FB950" if gain >= 0 else "#F85149", f"ROI {roi:+.1f}%"),
        (k4, "Flux Mensuel · Direct",   dir_cf,        "#3FB950", f"{dir_cf*12:,.0f} €/an"),
        (k5, "Flux Mensuel · Cascade",  casc_cf,       "#E3B341", f"{casc_cf*12:,.0f} €/an"),
        (k6, "Revenu Mensuel Total",    tot_cf,        "#79C0FF", f"{tot_cf*12:,.0f} €/an"),
    ]:
    ...
    with col_left:
        st.markdown('<div class="section-header">Participations SCI</div>', unsafe_allow_html=True)
        membership_rows = []
        for sci in nodes:
            ...
            membership_rows.append({
                ...
                "CF eff (€/mois)": cf_per_sci.get(sci, 0),
            })
        ...
        try:
            ...
            if sci_w:
                ...
                fig_wb.update_layout(plot_bgcolor="#0D1117", paper_bgcolor="#0D1117", font_color="#C9D1D9",
                    title=dict(text="Richesse par SCI (directe)", font_size=12),
    ...
    with col_mid:
        st.markdown('<div class="section-header">Flux — % effectif × pool effectif</div>', unsafe_allow_html=True)
        try:
            import plotly.graph_objects as go
            ...
            if cf_sci_labels:
                ...
                fig_donut.update_layout(plot_bgcolor="#0D1117", paper_bgcolor="#0D1117", font_color="#C9D1D9",
                    showlegend=False, title=dict(text="Flux par SCI source (% eff.)", font_size=12, x=0.5),
            ...
            if cf_sci_all:
                ...
                fig_cfbar.update_layout(plot_bgcolor="#0D1117", paper_bgcolor="#0D1117", font_color="#C9D1D9",
                    title=dict(text="Flux mensuel par SCI (% eff. × pool eff.)", font_size=12),
                    yaxis=dict(gridcolor="#21262D", title="€/mois"), xaxis=dict(gridcolor="#21262D", tickangle=-30),
                    margin=dict(t=35, b=70, l=55, r=10), height=245)
                st.plotly_chart(fig_cfbar, use_container_width=True)
        except ImportError: pass
    # ── Rendement Cash Annuel sur Investissement ───────────────────────────────────
    st.markdown('<div class="section-header">Rendement du Flux de Trésorerie sur Investissement</div>',
                unsafe_allow_html=True)
    ...
    with r1:
        r1.markdown(f"""<div class='metric-card'>
            <div class='label'>Flux de Trésorerie Annuel</div>
            <div class='value' style='color:#3FB950'>{annual_cf:,.0f} €</div>
            <div class='sub'>{tot_cf:,.0f} €/mois × 12</div></div>""", unsafe_allow_html=True)
    with r2:
        r2.markdown(f"""<div class='metric-card'>
            <div class='label'>Total Investi</div>
    ...
    with r3:
        ...
        r3.markdown(f"""<div class='metric-card'>
            <div class='label'>Rendement Cash (annuel)</div>
            <div class='value' style='color:{roi_color}'>{cash_roi:.2f}%</div>
            <div class='sub'>≈ {years_to_recoup:.1f} ans pour récupérer l'investissement</div></div>""",
                    unsafe_allow_html=True)

    # Décomposition par SCI
    try:
        import plotly.graph_objects as go
        ...
        if inv_by_sci:
            ...
            for s in inv_by_sci:
                ...
                rows_roi.append({
                    ...
                    "Flux / mois (€)": cf_mo_s,
                    "Flux / an (€)": cf_yr_s,
                    "Rendement Cash (%)": roi_s,
                    "Récupération (ans)": round(recoup_s, 1) if recoup_s != float("inf") else "∞",
                })
            ...
            total_roi_row = pd.DataFrame([{
                ...
                "Flux / mois (€)": tot_cf,
                "Flux / an (€)": annual_cf,
                "Rendement Cash (%)": cash_roi,
                "Récupération (ans)": round(years_to_recoup, 1) if years_to_recoup != float("inf") else "∞",
            }])
            ...
            st.dataframe(
                df_roi.style.format({
                    ...
                    "Flux / an (€)": "{:,.2f}",
                    "Rendement Cash (%)": "{:.2f}%",
                }).background_gradient(subset=["Rendement Cash (%)"], cmap="RdYlGn"),
            ...
            fig_roi = go.Figure(go.Bar(
                x=df_plot["SCI"],
                y=df_plot["Rendement Cash (%)"],
                marker_color=["#3FB950" if v >= 5 else "#E3B341" if v >= 2 else "#F85149"
                              for v in df_plot["Rendement Cash (%)"]],
                text=[f"{v:.2f}%" for v in df_plot["Rendement Cash (%)"]],
                textposition="outside", textfont_size=11,
            ))
            fig_roi.add_hline(y=cash_roi, line_dash="dot", line_color="#58A6FF",
                              annotation_text=f"Moyenne {cash_roi:.2f}%",
                              annotation_position="top right")
            fig_roi.update_layout(
                plot_bgcolor="#0D1117", paper_bgcolor="#0D1117", font_color="#C9D1D9",
                yaxis=dict(gridcolor="#21262D", title="Rendement cash annuel %",
                           zeroline=True, zerolinecolor="#444C56"),
                xaxis=dict(gridcolor="#21262D"),
                title=dict(text="Rendement cash annuel par SCI  =  (Flux × 12) / investi × 100", font_size=12),
                margin=dict(t=45, b=40, l=60, r=20), height=260,
            )
            st.plotly_chart(fig_roi, use_container_width=True)

    except ImportError:
        st.info("Installez plotly pour le graphique de rendement.")
    # st.markdown("---")
    ...
    #     else:
    #         st.info("Aucune donnée d'investissement initial trouvée pour cet associé.")
    # except ImportError: st.info("Installez plotly.")

# ── Graph Tab ──────────────────────────────────────────────────────────────────
with tab_graph:
    ctrl1, ctrl2, ctrl3 = st.columns([1.3, 1.3, 3])
    with ctrl1: show_associés  = st.toggle("👥 Show associés",    value=True)
    with ctrl2: show_effective = st.toggle("📈 Afficher % effectif", value=False)
    with ctrl3:
        if show_effective: st.caption("Trait plein = direct · tirets = indirect uniquement · % = effectif")
        else: st.caption("Affichage des parts directes uniquement")
    col_g1, col_g2 = st.columns([4, 1])
    with col_g2:
        st.markdown('<div class="section-header">Légende</div>', unsafe_allow_html=True)
        st.markdown("""<div style='font-size:.8rem;color:#8B949E;line-height:1.9'>
        <span style='color:#58A6FF'>●</span> SCI (taille = capitaux)<br/>
        <span style='color:#E3B341'>●</span> Associé (taille = richesse)<br/>
        <span style='color:#3FB950'>—</span> Part directe<br/>
        <span style='color:#E3B341'>┄</span> Indirect uniquement</div>""", unsafe_allow_html=True)
        st.markdown('<div class="section-header" style="margin-top:1rem">Capitaux SCI</div>', unsafe_allow_html=True)
        for node in nodes:
    ...
    with col_g1:
        net = build_pyvis(nodes, edges, equity_vec, dir_assets, sci_shares,
                          show_associés=show_associés, show_effective=show_effective,
