...
DEFAULT_ASSETS = """LEM IMMO : 1000000
M2N IMMO : 1000000
LEM2 IMMO : 500000
LEM SYNERGIE : 300000
LEM VIRY : 400000
...
# ── Tabs ──────────────────────────────────────────────────────────────────────
tab_input, tab_results, tab_cashflow, tab_profile, tab_graph = st.tabs([
    "📋 Data Input", "📊 SCI Equity", "💶 Cash Flow", "🔍 Partner Profile", "🕸️ Relationship Graph"
])
...
with tab_profile:
    ...
    # ── Group-level header KPIs ────────────────────────────────────────────────
    total_person_wealth = person_wealth.sum()
    total_direct_assets = sum(dir_assets.values())
    rounding_gap        = abs(total_direct_assets - total_person_wealth)
    richest_p           = all_persons[int(np.argmax(person_wealth))]
    top_earner_p        = all_persons[int(np.argmax(person_total_cf))]

    g1, g2, g3, g4, g5 = st.columns(5)
    for col, label, val, color, sub in [
        (g1, "Group Real Assets",     total_direct_assets,                                   "#58A6FF", f"{len(all_persons)} partners"),
        (g2, "Total Partner Wealth",  total_person_wealth,                                   "#3FB950", f"Gap {rounding_gap:.0f}\u20ac rounding"),
        (g3, "Largest Stakeholder",   person_wealth[P_idx[richest_p]],                       "#79C0FF", richest_p),
        (g4, "Group Monthly CF Pool", sum(v["to_partners"] for v in sci_cf.values()),        "#E3B341", f"{sum(v['to_partners'] for v in sci_cf.values())*12:,.0f} \u20ac/yr"),
        (g5, "Top Earner",            person_total_cf[P_idx[top_earner_p]],                  "#A371F7", top_earner_p + " /mo"),
    ]:
        col.markdown(f"""<div class='metric-card'>
            <div class='label'>{label}</div>
            <div class='value' style='color:{color};font-size:1.25rem'>{val:,.0f} \u20ac</div>
            <div class='sub'>{sub}</div></div>""", unsafe_allow_html=True)

    st.markdown("---")

    # ── Partner selector + rank badges ────────────────────────────────────────
    sorted_by_wealth = sorted(all_persons, key=lambda p: -person_wealth[P_idx[p]])
    sel_col, rank_col = st.columns([2, 4])
    with sel_col:
        selected_person = st.selectbox(
            "Select a partner", options=sorted_by_wealth,
            format_func=lambda p: f"{p}  \u2014  {person_wealth[P_idx[p]]:,.0f} \u20ac",
            label_visibility="collapsed", key="profile_selector",
        )
    ...
    rank_w  = sorted_by_wealth.index(selected_person) + 1
    rank_cf = sorted_by_cf.index(selected_person) + 1
    ...
    with rank_col:
        st.markdown(f"""<div style='display:flex;gap:.6rem;align-items:center;padding:.4rem 0;flex-wrap:wrap'>
          <span style='background:#0D1F3C;border:1px solid #1F6FEB;border-radius:8px;padding:.35rem .8rem;font-size:.82rem;color:#58A6FF'>\U0001f3c6 Wealth rank <b>#{rank_w}</b> / {len(all_persons)}</span>
          <span style='background:#0F2918;border:1px solid #238636;border-radius:8px;padding:.35rem .8rem;font-size:.82rem;color:#3FB950'>\U0001f4b6 CF rank <b>#{rank_cf}</b> / {len(all_persons)}</span>
          <span style='background:#1A1600;border:1px solid #BB8009;border-radius:8px;padding:.35rem .8rem;font-size:.82rem;color:#E3B341'>\U0001f3d7 <b>{len(direct_scis)}</b> direct \u00b7 <b>{len(eff_stakes)}</b> effective SCIs</span>
          <span style='background:#1A0D2E;border:1px solid #6E40C9;border-radius:8px;padding:.35rem .8rem;font-size:.82rem;color:#A371F7'>\U0001f4c5 {tot_cf*12:,.0f} \u20ac/yr</span>
        </div>""", unsafe_allow_html=True)

    # ── 8 KPI cards ────────────────────────────────────────────────────────────
    k1, k2, k3, k4 = st.columns(4)
    k5, k6, k7, k8 = st.columns(4)
    max_pw = max(person_wealth)
    max_cf = max(person_total_cf)
    w_share = pw / total_direct_assets * 100
    direct_equity = sum(d * equity_vec[idx[s]] for s, d in direct_scis.items())

    kpi_data = [
        (k1, "Total Wealth",            pw,                                   "#58A6FF", f"#{rank_w} \u00b7 {w_share:.2f}% of group",     "\u20ac"),
        (k2, "Equity (direct stakes)",  direct_equity,                        "#3FB950", f"{len(direct_scis)} SCI(s) direct",              "\u20ac"),
        (k3, "Monthly Cash \u00b7 Direct",  dir_cf,                          "#3FB950", f"{dir_cf*12:,.0f} \u20ac/yr",                    "\u20ac"),
        (k4, "Monthly Cash \u00b7 Cascade", casc_cf,                         "#E3B341", f"{casc_cf*12:,.0f} \u20ac/yr",                   "\u20ac"),
        (k5, "Total Monthly Income",    tot_cf,                               "#79C0FF", f"{tot_cf*12:,.0f} \u20ac/yr \u00b7 #{rank_cf}", "\u20ac"),
        (k6, "Cascade / Total ratio",   casc_cf/tot_cf*100 if tot_cf>0 else 0,"#A371F7", "% income from indirect chains",                   "%"),
        (k7, "Wealth vs richest",        pw/max_pw*100,                       "#58A6FF", f"vs {sorted_by_wealth[0]}",                        "%"),
        (k8, "CF vs top earner",         tot_cf/max_cf*100 if max_cf>0 else 0,"#E3B341", f"vs {sorted_by_cf[0]}",                            "%"),
    ]
    for col, label, val, color, sub, unit in kpi_data:
        fmt = f"{val:,.0f} {unit}" if unit == "\u20ac" else f"{val:.1f}{unit}"
        col.markdown(f"""<div class='metric-card'>
            <div class='label'>{label}</div>
            <div class='value' style='color:{color};font-size:1.1rem'>{fmt}</div>
            <div class='sub'>{sub}</div></div>""", unsafe_allow_html=True)

    st.markdown("---")

    # ── Main 3-column layout ──────────────────────────────────────────────────
    col_left, col_mid, col_right = st.columns([1.1, 1.1, 1.4], gap="large")
    ...
    with col_left:
        ...
        for sci in nodes:
            ...
            membership_rows.append({
                "SCI":          sci,
                "Type":         "Direct" if d_pct > 1e-9 else "Indirect",
                "Direct %":     d_pct * 100,
                "Effective %":  e_pct * 100,
        ...
        def _row_style(row):
            bg = "background-color:#0F2918" if row["Type"]=="Direct" else "background-color:#1A1600"
            return [bg]*len(row)
        st.dataframe(
            df_mem.style.apply(_row_style, axis=1).format({
                "Direct %":       "{:.3f}%",
                "Effective %":    "{:.4f}%",
                "My Claim (\u20ac)": "{:,.0f}",
                "CF (\u20ac/mo)":    "{:,.0f}",
        ...
        try:
            import plotly.graph_objects as go
            sci_w = [(s, d*equity_vec[idx[s]]) for s,d in sorted(direct_scis.items(), key=lambda x:-x[1])]
            if sci_w:
                fig_wb = go.Figure(go.Bar(
                    x=[s for s,_ in sci_w], y=[v for _,v in sci_w],
                    marker_color="#1F6FEB",
                    text=[f"{v:,.0f}\u20ac" for _,v in sci_w],
                ...
                fig_wb.update_layout(
                    ...
                    xaxis=dict(gridcolor="#21262D"),
                    margin=dict(t=35,b=30,l=55,r=10), height=190,
                )
        ...
        try:
            ...
            if cf_labels:
                fig_donut = go.Figure(go.Pie(
                    labels=cf_labels, values=cf_vals, hole=0.55,
                ...
                fig_donut.update_layout(
                    plot_bgcolor="#0D1117", paper_bgcolor="#0D1117", font_color="#C9D1D9",
                    showlegend=False,
                    title=dict(text="Monthly CF by source SCI", font_size=12, x=0.5),
                    annotations=[dict(text=f"{tot_cf:,.0f}\u20ac<br>/mo", x=0.5, y=0.5,
                                      font_size=12, showarrow=False, font_color="#C9D1D9")],
                    margin=dict(t=40,b=5,l=5,r=5), height=235,
                )
                st.plotly_chart(fig_donut, use_container_width=True)

            fig_dcb = go.Figure()
            fig_dcb.add_trace(go.Bar(name="Direct",  x=["Monthly","Annual"], y=[dir_cf,  dir_cf*12],  marker_color="#3FB950"))
            fig_dcb.add_trace(go.Bar(name="Cascade", x=["Monthly","Annual"], y=[casc_cf, casc_cf*12], marker_color="#E3B341"))
            fig_dcb.update_layout(
                barmode="stack", plot_bgcolor="#0D1117", paper_bgcolor="#0D1117",
                font_color="#C9D1D9", font_family="Inter",
                legend=dict(orientation="h", y=1.08, x=0, font_size=10),
                yaxis=dict(gridcolor="#21262D", title="\u20ac"),
                xaxis=dict(gridcolor="#21262D"),
                title=dict(text="Direct vs cascade", font_size=12),
                margin=dict(t=40,b=30,l=55,r=10), height=185,
            )
            st.plotly_chart(fig_dcb, use_container_width=True)

            st.markdown('<div class="section-header" style="margin-top:.5rem">CF Rank vs All Partners</div>', unsafe_allow_html=True)
            colors_rank = ["#F85149" if p==selected_person else "#388BFD" for p in sorted_by_cf]
            fig_rank = go.Figure(go.Bar(
                x=sorted_by_cf,
                y=[person_total_cf[P_idx[p]] for p in sorted_by_cf],
                marker_color=colors_rank,
                text=[f"{person_total_cf[P_idx[p]]:,.0f}" for p in sorted_by_cf],
                textposition="outside", textfont_size=8,
            ))
            fig_rank.update_layout(
                plot_bgcolor="#0D1117", paper_bgcolor="#0D1117", font_color="#C9D1D9",
                yaxis=dict(gridcolor="#21262D", title="\u20ac/mo"),
                xaxis=dict(gridcolor="#21262D", tickangle=-40, tickfont_size=8),
                margin=dict(t=10,b=70,l=55,r=10), height=220,
            )
    ...
    with col_right:
        st.markdown('<div class="section-header">Wealth Evolution Projection</div>', unsafe_allow_html=True)
        try:
            import plotly.graph_objects as go
            from plotly.subplots import make_subplots
            ev_c1, ev_c2 = st.columns(2)
            with ev_c1:
                growth_rate = st.slider("Annual growth %", 0.0, 15.0, 3.0, 0.5, key="profile_growth")
            with ev_c2:
                months_horizon = st.select_slider("Horizon", [12,24,36,60,120], 60,
                                                   format_func=lambda x: f"{x//12}yr", key="profile_horizon")
            ...
            fig_evo.add_trace(go.Scatter(x=months, y=equity_curve, name="SCI Equity",
                mode="lines", fill="tozeroy", line=dict(color="#1F6FEB",width=2),
                fillcolor="rgba(31,111,235,0.15)"), row=1, col=1)
            fig_evo.add_trace(go.Scatter(x=months, y=total_curve, name="+ Accumulated cash",
                mode="lines", fill="tonexty", line=dict(color="#3FB950",width=2),
                fillcolor="rgba(63,185,80,0.12)"), row=1, col=1)
            fig_evo.add_trace(go.Bar(x=months, y=[tot_cf]*len(months),
                name="Monthly income", marker_color="#E3B341", opacity=0.65), row=2, col=1)
            for m in range(0, months_horizon+1, 12):
                fig_evo.add_vline(x=m, line_dash="dot", line_color="#30363D", line_width=1)
            fig_evo.update_layout(
                plot_bgcolor="#0D1117", paper_bgcolor="#0D1117", font_color="#C9D1D9",
                legend=dict(orientation="h", y=1.03, x=0, font_size=10),
                margin=dict(t=30,b=40,l=60,r=10), height=390,
            )
            fig_evo.update_xaxes(tickvals=[m for m in months if m%12==0],
                                  ticktext=[f"Y{m//12}" for m in months if m%12==0],
                                  gridcolor="#21262D")
            fig_evo.update_yaxes(gridcolor="#21262D", tickformat=",.0f", title_text="\u20ac", row=1, col=1)
            fig_evo.update_yaxes(gridcolor="#21262D", tickformat=",.0f", title_text="\u20ac/mo", row=2, col=1)
            ...
            for m in range(0, months_horizon+1, 12):
                milestone_rows.append({
                    "Year": f"Y{m//12}",
                    "Equity (\u20ac)":           equity_curve[m],
                    "Cash accumulated (\u20ac)":  cash_curve[m],
                    "Total wealth (\u20ac)":      total_curve[m],
                    "Growth":                      f"+{(total_curve[m]/pw-1)*100:.1f}%",
                })
            st.dataframe(
                pd.DataFrame(milestone_rows).style
                  .format({"Equity (\u20ac)":"{:,.0f}","Cash accumulated (\u20ac)":"{:,.0f}","Total wealth (\u20ac)":"{:,.0f}"})
                  .background_gradient(subset=["Total wealth (\u20ac)"], cmap="Blues"),
                use_container_width=True, hide_index=True, height=210,
            )
        except ImportError:
            st.info("Install plotly for evolution charts.")

    with st.expander("\U0001f4cb Full partners comparison table"):
        table_rows = []
        for person in sorted_by_wealth:
            pw_p  = person_wealth[P_idx[person]]
            tcf_p = person_total_cf[P_idx[person]]
            dcf_p = person_direct_cf[P_idx[person]]
            ccf_p = person_indirect_cf[P_idx[person]]
            sci_h = person_sci_direct.get(person, {})
            table_rows.append({
                "Partner":          person,
                "Total Wealth (\u20ac)": pw_p,
                "% of Group":       pw_p/total_direct_assets*100,
                "Monthly CF (\u20ac)":   tcf_p,
                "  Direct (\u20ac)":     dcf_p,
                "  Cascade (\u20ac)":    ccf_p,
                "Annual CF (\u20ac)":    tcf_p*12,
                "Direct SCIs":      ", ".join(f"{s} {p*100:.1f}%" for s,p in sorted(sci_h.items(), key=lambda x:-x[1])),
            })
        df_full = pd.DataFrame(table_rows)
        st.dataframe(
            df_full.style.format({
                "Total Wealth (\u20ac)": "{:,.0f}", "% of Group": "{:.2f}%",
                "Monthly CF (\u20ac)": "{:,.0f}", "  Direct (\u20ac)": "{:,.0f}",
                "  Cascade (\u20ac)": "{:,.0f}", "Annual CF (\u20ac)": "{:,.0f}",
            })
            .background_gradient(subset=["Total Wealth (\u20ac)"], cmap="Blues")
            .background_gradient(subset=["Monthly CF (\u20ac)"],   cmap="Greens")
            .highlight_max(subset=["Total Wealth (\u20ac)","Monthly CF (\u20ac)"], color="#0F2918"),
            use_container_width=True, hide_index=True,
        )
# ── Graph Tab ─────────────────────────────────────────────────────────────────
with tab_graph:
